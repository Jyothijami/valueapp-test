...
public partial class BATMP1 : System.Web.UI.MasterPage
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            string EmpId = Session["vl_empid"].ToString();
            ...
            Literal1.Text = "<a href='/Digitalcard/Vcard.aspx?Cid=" + EmpId + "'>e-card</a>";
            ...
            int noticountReward = 0;
            noticountReward = HR.Reward.getCircularRewardCount(userid);
            if (noticountReward > 0)
            {
                litnotocountreward.Text = "<span class='badge badge-important'>" + noticountReward.ToString() + "</span>";
            }
    ...
    {
        con.Open();
        SqlCommand cmd = new SqlCommand("select EMP_FIRST_NAME from YANTRA_EMPLOYEE_MAST , YANTRA_EMPLOYEE_DET where YANTRA_EMPLOYEE_MAST .EMP_ID =YANTRA_EMPLOYEE_DET .EMP_ID  and DATEPART(M,EMP_DOB)=DATEPART(m,getdate()) and DATEPART(d,EMP_DOB)=DATEPART(d,getdate()) and status=1", con);
    ...
    {
        Response.Redirect("/Greetings/NewGreet.aspx");
