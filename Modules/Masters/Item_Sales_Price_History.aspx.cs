...
public partial class Modules_Masters_Item_Sales_Price_History : basePage
{
    ...
    private void BindHistoryGrid()
    {
        if (txtModelNo.Text != "")
        {
            SqlCommand cmd = new SqlCommand("USP_ItemPriceHistory4", con);
        ...
        {
            SqlCommand cmd = new SqlCommand("USP_ItemPriceHistory3", con);
            cmd.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
    ...
    {
        User_Permissions up = new User_Permissions(Session["vl_userid"].ToString(), "110");
    ...
    {
        SM.DDLBindWithSelect(ddlModelNo, "select DISTINCT YANTRA_ITEM_MAST.ITEM_CODE,YANTRA_ITEM_MAST.ITEM_MODEL_NO from YANTRA_ITEM_MAST,YANTRA_ITEM_PRICE  where YANTRA_ITEM_PRICE.SubCat_Id = YANTRA_ITEM_MAST.IT_TYPE_ID  and YANTRA_ITEM_PRICE.Brand_id=YANTRA_ITEM_MAST.BRAND_ID "+
"and YANTRA_ITEM_MAST.IC_ID=YANTRA_ITEM_PRICE.Cat_Id and YANTRA_ITEM_PRICE.SubCat_Id = '" + ddlSubCategory.SelectedItem.Value + "' and YANTRA_ITEM_PRICE.Brand_id='" + ddlBrand.SelectedItem.Value + "' and YANTRA_ITEM_PRICE.Cat_Id='" + ddlCategory.SelectedItem.Value + "'  ");
    }
    protected void btnBrandUpdate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (txtAmount.Text != "" && txtPercentage.Text == "")
            {
                ...
                {
                    Masters.ItemPurchase.ItemModel_RateCalc(ddlModelNo.SelectedValue, txtPercentage.Text, Convert.ToDecimal(txtAmount.Text));
                }
    ...
    {
        Masters.ItemCategory.ItemCategory_Select_WithBrand(ddlCategory, ddlBrand.SelectedItem.Value);
    ...
    protected void gvItemPriceHistory_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            e.Row.Cells[1].Text = "Page " + (gvItemPriceHistory.PageIndex + 1) + " of " + gvItemPriceHistory.PageCount;
        }
    }
}
