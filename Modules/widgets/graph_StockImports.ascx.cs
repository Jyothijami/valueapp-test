...
[PartialCaching(300)]
public partial class Modules_widgets_graph_StockImports : System.Web.UI.UserControl
{
    DataTable smdt;
    protected void Page_Load(object sender, EventArgs e)
    {
        smdt = stock.getStockImportCount_Monthly();
    }
    ...
    protected string getMDataCount(string MonthName)
    {
        ...
        try
        {
            ...
            {
                x = x + Convert.ToInt32(dr["Quantity"]);
            }
        }
        catch (Exception)
        {
    ...
    protected string getMData(string MonthName)
    {
        ...
        try
        {
            ...
            foreach (DataRow dr in selectedTable.Rows)
            {
                ...
                st = st + "['" + dr["Brand"].ToString() + "'," + dr["Quantity"].ToString() + "]";
            }
        }
        catch (Exception) { }
        return st;
