...
SqlConnection con = dbc.con;
	...
	{
        //mark_read(Request.QueryString["CIR_ID"]);
	}
	...
	{
        if (msgs.is_msg_folder_inbox(Request.QueryString["CIR_ID"], Session["vl_userid"].ToString()))
        {
            send_mesg_panel1.Visible = true;
            back_folder_hypl51.Text = "Back to inbox";
            back_folder_hypl51.NavigateUrl = "my_inbox.aspx";

            string uid12 = msgs.get_msg_uid(Request.QueryString["CIR_ID"]);
	...
	{
		bool is_inbox = false;
        is_inbox = msgs.is_msg_folder_inbox(Request.QueryString["CIR_ID"], Session["vl_userid"].ToString());
        ...
        if (is_inbox)
        {
            ...
            cmd.Parameters.Add("@msgid", SqlDbType.VarChar).Value = Request.QueryString["CIR_ID"];
        }
        else
        {
    ...
    {
        Response.Redirect("~/Modules/home/TimeSheet.aspx?t=Forward&msgid=" + Request.QueryString["CIR_ID"]);
    }

    protected void delmesglkbt1_Click(object sender, EventArgs e)
    {
        if (msgs.delete_message(Request.QueryString["CIR_ID"]))
        {
            Response.Redirect("my_inbox.aspx");
        }
    }
