...
public partial class Modules_SM_SalesOrder : basePage
{
    ...
    private void setControlsVisibility()
    {
        ...
        btnAcceptence.Enabled = up.Approve;
        //btnSend.Enabled = up.Email;
        btnPrint.Enabled = up.Print;
        //btnClose.Enabled = up.Close;
        //GridViewRow gvSalesOrderDetails = gvSalesOrderDetails.
        Button btnMC = (Button)gvSalesOrderDetails.FindControl("btnAcceptence");
        //btnMC.Enabled = up.Approve;
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        if (gvSalesOrderDetails.SelectedIndex > -1)
        {
            if (!string.IsNullOrEmpty(gvSalesOrderDetails.SelectedRow.Cells[7].Text) && gvSalesOrderDetails.SelectedRow.Cells[7].Text != "&nbsp;")
            {
                ...
                if (gvSalesOrderDetails.SelectedRow.Cells[7].Text == "Obsolete")
                {
                    btnAcceptence.Visible = false;
                    btnApprove.Visible = true;
                    btnSendWorkOrder.Visible = false;
                    btnSend.Visible = true;
                    
                }
                else if (gvSalesOrderDetails.SelectedRow.Cells[7].Text == "ManuallyClosed")
                {
                    ...
                    if (Uploadattach.HasFiles)
                    {
                        ...
                        if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "/YANTRA_DOCUMENTS/SOFiles"))
                        {
                            ...
                            foreach (HttpPostedFile uploadedFile in Uploadattach.PostedFiles)
                            {
                                ...
                                string randNumber = Convert.ToString(rand.Next(10000, 90000));
            ...
            if (objSM.SalesOrder_Update() == "Data Updated Successfully")
            {
                ...
                if (Uploadattach.HasFiles)
                {
                    ...
                    if (Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "/YANTRA_DOCUMENTS/SOFiles"))
                    {
                        ...
                        {
                            Random rand = new Random();
                            string randNumber = Convert.ToString(rand.Next(10000, 80000));
                            string path = Server.MapPath("~/YANTRA_DOCUMENTS/SOFiles/");
                            string fileName = System.IO.Path.GetFileName(uploadedFile.FileName);
    ...
    protected void gvSalesOrderDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow || e.Row.RowType == DataControlRowType.Header)
        {
            ...
            e.Row.Cells[13].Visible = false;
            if (lblEmpIdHidden.Text == "50" || lblEmpIdHidden.Text == "314" || lblEmpIdHidden .Text =="44")
            {
    ...
    protected void btnApprove_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SM.SalesOrder.SalesOrderStatus_Update(SM.SMStatus.Obsolete, gvSalesOrderDetails.SelectedRow.Cells[0].Text);
            SM.CommitTransaction();
        }
        catch (Exception ex)
        {
    ...
    protected void btnAcceptence_Click(object sender, EventArgs e)
    {
         try
        {
            ...
            objSMSOApprove.SOApprovedBy = Yantra.Authentication.GetEmployeeInSession(Yantra.Authentication.Logged_EMP_Details.EmpId);
            objSMSOApprove.SalesOrderApprove_Update();
            SM.SalesOrder.SalesOrderStatus_Update(SM.SMStatus.ManuallyClosed , gvSalesOrderDetails.SelectedRow.Cells[0].Text);
            SM.CommitTransaction();
        }
        catch (Exception ex)
        {
    ...
    protected void btnUpdatePO_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                CheckBox ch = new CheckBox();
                ch = (CheckBox)gvr.FindControl("chkItemSelect");
                if (ch.Checked == true)
            
            {
                TextBox rate = (TextBox)gvr.FindControl("txtMRP") as TextBox;
                TextBox qty = (TextBox)gvr.FindControl("txtQuantity") as TextBox;
                Label UnitPrice = (Label)gvr.FindControl("lblUnitPrice") as Label;
                Label splPrice = (Label)gvr.FindControl("lblSplPrice") as Label;
                //TextBox Disount = (TextBox)gvr.FindControl("txtgvDiscount") as TextBox;
                TextBox BalQty = (TextBox)gvr.FindControl("txtBalanceQty") as TextBox;
                TextBox AnexQty = (TextBox)gvr.FindControl("txtAnnexureQty") as TextBox;

                if (qty.Text != "" && rate.Text != "")
                {
                    if (gvr.RowType == DataControlRowType.DataRow)
                    {
                        objsm.SOItemCode = gvr.Cells[2].Text;
                        objsm.ColorId = gvr.Cells[15].Text;
                        objsm.SODetQty = qty.Text;
                        objsm.SORate = rate.Text;
                        objsm.SODetPrice = splPrice.Text;
                        objsm.BalanceQty = BalQty.Text;
                        objsm.AnnexureQty = AnexQty.Text;
                        objsm.SOAmmendment_Update(gvr.Cells[18].Text);
    ...
    protected void gvSalesOrderDetails_RowCommand(object sender, GridViewCommandEventArgs e)
    {
       

        try
        {
            if (e.CommandName.Equals("Manual Close"))
            {
            ...
            SM.SalesOrder objSMSOApprove = new SM.SalesOrder();
            SM.BeginTransaction();
            objSMSOApprove.SOId = rowindex.ToString();
            objSMSOApprove.SOApprovedBy = Yantra.Authentication.GetEmployeeInSession(Yantra.Authentication.Logged_EMP_Details.EmpId);
            objSMSOApprove.SalesOrderApprove_Update();
            SM.SalesOrder.SalesOrderStatus_Update(SM.SMStatus.ManuallyClosed, rowindex.ToString());
            SM.CommitTransaction();
            }
            else if (e.CommandName.Equals("FPS"))
            {
                int rowindex = int.Parse(e.CommandArgument.ToString().Trim());
                string SOID = rowindex.ToString();
                Response.Redirect("FPS.aspx?SOId=" + SOID);

            }
            else if (e.CommandName.Equals("Statement"))
            {
                int rowindex = int.Parse(e.CommandArgument.ToString().Trim());
                string SOID = rowindex.ToString();
                string pagenavigationstr = "../Reports/SMReportViewer.aspx?type=StatementPO&sono=" + SOID + "";
                System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), "rptwindow", "window.open('" + pagenavigationstr + "','rptview','resizable=yes,scrool=yes,width=900,height=600,status=yes,toolbar=no,menubar=no,scrollbars=yes')", true);
