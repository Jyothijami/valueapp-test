...
public partial class Modules_SM_SalesQuotationDetails1 : basePage
{
    ...
    decimal TotalAmount = 0, gstAmount=0;
    ...
    private void SalesQuotationUpdate()
    {
        if (gvQuotationItems.Rows.Count > 0)
        {
            try
            {
                ...
                objSM.QuotRespId = ddlResponsiblePerson.SelectedItem.Value;
                objSM.QuotSalespId = ddlSalesPerson.SelectedItem.Value;
                objSM.QuotPreparedBy = ddlPreparedBy.SelectedItem.Value;
                objSM.QuotCheckedBy = Yantra.Authentication.GetEmployeeInSession(Yantra.Authentication.Logged_EMP_Details.EmpId);
                objSM.QuotApprovedBy = ddlApprovedBy.SelectedItem.Value;
                objSM.CurrencyId = ddlCurrencyType.SelectedItem.Value;
        ...
        if (Request.QueryString["QuoId"] != null)
        {
            try
            {
                ...
                if (objSM.SalesQuotation_Select(Request.QueryString["QuoId"].ToString()) > 0)
                {
                    ...
                    objSM.SalesQuotationDetails_SelectPreview(Request.QueryString["QuoId"].ToString(), gvQuotationItems);
    ...
    protected void gvQuotationItems_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            e.Row.Cells[11].Text = Math.Round(Convert.ToDecimal(e.Row.Cells[11].Text)).ToString ();
            e.Row.Cells[13].Text = Math.Round((Convert.ToDecimal(e.Row.Cells[11].Text) * Convert.ToDecimal(e.Row.Cells[12].Text)) / 100).ToString();
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            gstAmount = gstAmount + Convert.ToDecimal(e.Row.Cells[13].Text);

        }
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            e.Row.Cells[9].Text = "Total Amount:";
           lblTtlAmt .Text = e.Row.Cells[11].Text = TotalAmount.ToString();
           lblTtlGSTAmt.Text = e.Row.Cells[13].Text = gstAmount.ToString();
            e.Row.Cells[10].Visible = false;
            e.Row.Cells[15].Visible = false;
            e.Row.Cells[17].Visible = false;
            e.Row.Cells[0].Visible = false;
            ...
            { txtsubtotal.Text = Math .Round (Convert.ToDecimal(txttotal.Text) - (Convert.ToDecimal(txttotal.Text) * Convert.ToDecimal(txtspldiscount.Text)) / 100).ToString(); }
            txtsummaryvat.Text = Math.Round((Convert.ToDecimal(txtsubtotal.Text) * Convert.ToDecimal(18)) / 100).ToString();
            txtsummaryCst.Text = Math.Round((Convert.ToDecimal(txtsubtotal.Text) * Convert.ToDecimal(2)) / 100).ToString();
            txtsummaryGtoalvat.Text = Math.Round(Convert.ToDecimal(txtsubtotal.Text) + Convert.ToDecimal(txtsummaryvat.Text)).ToString();
            txtsummaryGtotalcst.Text = Math.Round(Convert.ToDecimal(txtsubtotal.Text) + Convert.ToDecimal(txtsummaryCst.Text)).ToString();
        }
    }
    #endregion
    ...
    protected void txtDisc_TextChanged(object sender, EventArgs e)
    {
        foreach (GridViewRow gvr in gvQuotationItems.Rows)
        {
            ...
            gvr.Cells[11].Text = Math.Round((Convert.ToDecimal(Rate.Text) - (Convert.ToDecimal(disc.Text) * Convert.ToDecimal(Rate.Text)) / 100) * Convert.ToDecimal(Qty.Text)).ToString();
            gvr.Cells[13].Text = Math.Round((Convert.ToDecimal(gvr.Cells[11].Text) * Convert.ToDecimal(gvr.Cells[12].Text)) / 100).ToString();
            TotalAmount = TotalAmount + Convert.ToDecimal(gvr.Cells[11].Text);
            gstAmount  = gstAmount  + Convert.ToDecimal(gvr.Cells[13].Text);


            lblTtlAmt.Text  = TotalAmount.ToString();
            lblTtlGSTAmt.Text = gstAmount.ToString();

            txttotal.Text = txtsubtotal.Text = lblTtlAmt.Text;
            if (txtspldiscount.Text == "") { txtspldiscount.Text = "0"; }
            if (txtspldiscount.Text != "0")
            { txtsubtotal.Text = Math.Round(Convert.ToDecimal(txttotal.Text) - (Convert.ToDecimal(txttotal.Text) * Convert.ToDecimal(txtspldiscount.Text)) / 100).ToString(); }
            txtsummaryvat.Text = Math.Round((Convert.ToDecimal(txtsubtotal.Text) * Convert.ToDecimal(18)) / 100).ToString();
    ...
    protected void btnDisc_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow gvrow in gvQuotationItems.Rows)
        {
            ...
            if (ch.Checked == true)
            {
                TextBox disc = (TextBox)gvrow.FindControl("txtDisc");
                disc.Text = txtItemDisc.Text;
                txtDisc_TextChanged(sender,e);
    ...
    protected string toEmail1, EmailSubj1, EmailMsg1, ccId1, bccId1;
    protected void btnNotif_Click(object sender, EventArgs e)
    {
        if (Request.QueryString["QuoId"] != null)
        {
            ...
            {
                toEmail1 = "sales@valueline.in";
                ccId1 = "it@alumil.in," + txtFollowupEmail.Text + " ";
                EmailSubj1 = "" + txtQuotationNo .Text  + " - " + ddlCustomer.SelectedItem .Text + "";
                StreamReader reader = new StreamReader(Server.MapPath("~/regular/index.htm"));
                string readFile = reader.ReadToEnd();
                string myString = "";
                myString = readFile;
                myString = myString.Replace("{Text}", "Dear Edp Team, you have been assigned for Quotation with the Quotation No : " + txtQuotationNo.Text + " - " + ddlCustomer.SelectedItem.Text + " By Sales Executive " + ddlResponsiblePerson.SelectedItem.Text + ",please go through the link below to more details ");
                EmailMsg1 = myString;
                SendEmail.Email_With_CCandBCC(toEmail1, ccId1, EmailSubj1, EmailMsg1);
            });
    ...
    protected void rdbGSTInclusive_CheckedChanged(object sender, EventArgs e)
    {
        string pagenavigationstr = "";
        if (rdbGSTInclusive.Checked == true)
        {

            //pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quot&qno=" + Request.QueryString["QuoId"].ToString() + "";
            //pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quotwinp&qno=" + Request.QueryString["QuoId"].ToString() + "";
            pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quotwinp_NoGST&qno=" + Request.QueryString["QuoId"].ToString() + "";
    ...
    protected void rdbWoprices_CheckedChanged(object sender, EventArgs e)
    {
        string pagenavigationstr = "";
        if (rdbWoprices.Checked == true)
        {

            //pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quot&qno=" + Request.QueryString["QuoId"].ToString() + "";
            //pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quotwinp&qno=" + Request.QueryString["QuoId"].ToString() + "";
            pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quotwinp_WoMRP&qno=" + Request.QueryString["QuoId"].ToString() + "";
    ...
    protected void rdbwocodeprices_CheckedChanged(object sender, EventArgs e)
    {
        string pagenavigationstr = "";
        if (rdbwocodeprices.Checked == true)
        {

            //pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quot&qno=" + Request.QueryString["QuoId"].ToString() + "";
            //pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quotwinp&qno=" + Request.QueryString["QuoId"].ToString() + "";
            pagenavigationstr = "../Reports/SMReportViewer.aspx?type=quotwinp_WoMRPcodes&qno=" + Request.QueryString["QuoId"].ToString() + "";
