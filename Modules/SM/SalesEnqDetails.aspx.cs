...
public partial class Modules_SM_SalesEnqDetails : basePage
{
    #region PAGE LOAD
    ...
    private void EmployeeMaster_Fill()
    {
        try
        {
            ...
            HR.EmployeeMaster.EmployeeMaster_SelectForSales(ddlOriginatedBy);
    ...
    protected void gvInterestedProducts_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (btnSave.Enabled == false)
        {
            if (e.Row.RowType == DataControlRowType.Header || e.Row.RowType == DataControlRowType.DataRow)
            {
                e.Row.Cells[0].Visible = e.Row.Cells[1].Visible = e.Row.Cells[9].Visible = false;
            }
        }
        if (e.Row.RowType == DataControlRowType.Header || e.Row.RowType == DataControlRowType.DataRow)
        {
           e.Row.Cells[2].Visible =e.Row.Cells[16].Visible =  e.Row.Cells[0].Visible = e.Row.Cells[1].Visible = e.Row.Cells[9].Visible = false;
        }
    ...
    private void SalesEnquirySave()
    {
        if (gvInterestedProducts.Rows.Count > 0)
        {
            try
            {
                ...
                objSM.EnqOrigName = ddlOriginatedBy.SelectedItem.Text;
                objSM.EnqRef = txtReferenceCode.Text;
                objSM.EnqFollowUp = txtFollowUpCriteria.Text;
                objSM.EnqDeliveryDate = DateTime.Now.ToString();
                objSM.DespModeId = "0";
                ...
                if (objSM.SalesEnquiry_Save() == "Data Saved Successfully")
                {
                    ...
                    foreach (GridViewRow gvrow in gvInterestedProducts.Rows)
                    {
                        ...
                        objSM.itemname = gvrow.Cells[4].Text;
                        objSM.brand = gvrow.Cells[5].Text;
    ...
    private void SalesEnquiryUpdate()
    {
        if (gvInterestedProducts.Rows.Count > 0)
        {
            try
            {
                ...
                objSM.EnqRef = txtReferenceCode.Text;
                objSM.EnqFollowUp = txtFollowUpCriteria.Text;
                ///Delivery Date used as Last Updated Date
                objSM.EnqDeliveryDate = DateTime.Now.ToString();
                objSM.DespModeId = "0";
                objSM.PromotionType = txtPromotionType.Text;
                objSM.PromotionActivity = txtPromotionActivity.Text;
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["Cid"] != null)
        {
            ...
            {
                SM.SalesEnquiry objSM = new SM.SalesEnquiry();
                MessageBox.Show(this, objSM.SalesEnquiry_Delete(Request.QueryString["Cid"].ToString()));
                Response.Redirect("SalesEnquiry.aspx");
            }
            catch (Exception ex)
            {
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        if (Request.QueryString["Cid"] != null)
        {
            string pagenavigationstr = "../Reports/SMReportViewer.aspx?type=salesenq&enqno=" + Request.QueryString["Cid"].ToString() + "";
    ...
    {
        tblAssignTasks.Visible = false;
        Response.Redirect("~/Modules/SM/SSalesEnquiry.aspx");
    }
    #endregion
    ...
    protected void btnRegret_Click(object sender, EventArgs e)
    {
        if (Request.QueryString["Cid"] != null)
        {
            try
            {
                MessageBox.Show(this, SM.SalesEnquiry.SalesEnquiryStatus_Update(SM.SMStatus.Obsolete, Request.QueryString["Cid"].ToString()));
            }
            catch (Exception ex)
            {
                MessageBox.Show(this, ex.Message);
