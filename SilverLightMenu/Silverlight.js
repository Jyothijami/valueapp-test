...
function createSilverlight()
{
	...
	Silverlight.createObjectEx({
		source: '../../SilverLightMenu/Scene.xaml',
...
function TimeCustomValidate(source,text)
{
    try
    {
        var checkbool,yearbool;
       if( text.Value=="" || text.Value=="__:__ AM" || text.Value=="__:__ PM")
       {
            return;
       }
        var time=text.Value;
        var arrvals=time.split(":");
        var hh=arrvals[0];
        arrvals=arrvals[1].split(" ");
        var mm=arrvals[0];
        var AMPM=arrvals[1];
       
        if(hh>=1 && hh<=12 && mm>=00 && mm<=59)
        {
            if(AMPM=="AM" && hh<9)
            {
                checkbool=false;
            }
            else if(AMPM=="PM" && hh>6)
            {
                checkbool=false;
            }
            else if(AMPM=="PM" && hh==6 && mm>00)
            {
                checkbool=false;
            }
        }
...
function DateCustomValidate(source,text)
{
    try
    {
        var checkbool,yearbool;
       if( text.Value=="" || text.Value=="__/__/____")
       {
            return;
       }
        var dt=text.Value;
...
////////function DueDateExceedValidate(source,text)
////////{
////////    var d=new Date();
////////    alert(d);
////////    var formatter=new formatter("dd/MM/yyyy");
////////alert(formatter.format(d));
////////alert(Date.parse(text.Value));
////////    if(Date.parse(text.Value)>=d)
////////    {
////////        text.IsValid =false;
////////    }
////////    else 
